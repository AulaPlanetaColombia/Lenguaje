...
Main = new function()
{
    ...
    this.continueInit = function(){
        ...
        Main.maxHeight = window.innerHeight ;
        Main.maxWidth = window.innerWidth; 
        Main.ratio =  Main.maxHeight / Main.maxWidth;

        if(Main.stage.canvas.height < window.innerHeight -20){
            Main.vertical = true;
        }else{
            Main.vertical = false;
        }
    ...
    this.windowResize =function ()
    {
       // browser viewport size
        ...
        // Miramos la verticalida actual
        if(Main.stage.canvas.height < h -20){
           var currentVertical = true;
        }else{
            var currentVertical = false;
        }

        //para forzar el blur del input y que no aparezca cada vez que se toca la pantalla
        if( Math.floor(Main.ratio) != Math.floor(h/w) && ( Main.vertical  != currentVertical ) ){
            //alert(Math.floor(Main.ratio) + " -- " +Math.floor(h/w));
            Main.maxHeight = window.innerHeight ;
            Main.maxWidth = window.innerWidth;
            Main.ratio = Main.maxHeight / Main.maxWidth;
        }else{
            if( indexCR > -1 && mobil == "Android" &&  ( h == Main.maxHeight )){
                $('#inputar').blur();
            }
        }

        // Calculamos la verticalidad para el resize proximo
        if(Main.stage.canvas.height < h -20){
            Main.vertical = true;
        }else{
            Main.vertical = false;
